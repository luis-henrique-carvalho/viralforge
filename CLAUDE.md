...
- **Frontend Web (web/) — 100% Shadcn-First Compliance**: Always compose UI features from official Shadcn UI primitives located in `@/components/ui/*` (`Button`, `Input`, `Badge`, `Card`, `Switch`, `Select`, `Dialog`, `Sheet`, `ScrollArea`, `Separator`, etc.). Never create raw HTML buttons/inputs or custom primitive widgets that duplicate Shadcn functionality. If a specialized UI component is truly needed that cannot be built with standard Shadcn components, you MUST explicitly request user authorization before creating it. Every frontend feature must pass `./scripts/check_shadcn_usage.py` with 0 warnings.
- **Frontend Quality**: Respect max 80 lines per function, max 200 lines per component, zero `any` in TypeScript, and strict TanStack Router / TanStack Query separation (`views/`, `components/`, `hooks/`, `services/`, `data/`).
