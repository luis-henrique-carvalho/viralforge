...
describe('TemplateAiTestSection', () => {
  it('renders test prompt input and executes generation test', async () => {
    renderWithProviders(<TemplateAiTestSection template={mockTemplate} />)

    expect(screen.getByText('Testar Geração em Tempo Real')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Executar Teste/i })).toBeInTheDocument()

    const runBtn = screen.getByRole('button', { name: /Executar Teste/i })
    fireEvent.click(runBtn)

    await waitFor(() => {
      expect(screen.getByText(/Resposta Sintetizada com Sucesso/i)).toBeInTheDocument()
    })
  })
})
